...
- 本地优先：主要识别过程在本地完成，无需接入大模型，不消耗token
- 三种输入：音频文件、视频文件、实时录音（实时录音还有点bug=。=
- 长内容友好：适合会议、课程、访谈和播客这类真实素材。效率还不错（转过120分钟音频，速度挺快的，具体时间忘了，下次再更新上来=。=
- 三种输出：一键复制，支持 TXT 和 Markdown 导出
- 可选增强：支持说话人识别，可配置 LLM 做轻量纠错
...
3. 点击 `Start`。
4. 等待本地转录完成后复制或导出结果。
5. 如需 LLM 纠错，点击 `Refine`。纠错完成后，界面中的文本会替换为纠错后的版本。
...
2. 下载最新的 `VoxNote-0.1.4-macOS.dmg`。
...
LLM 纠错是可选功能，默认关闭。只有在你主动配置、开启，并点击 `Refine` 后，文本才会发送到你填写的外部接口。

## 开源协议

VoxNote 使用 MIT License。第三方库和模型说明见 [第三方声明](THIRD_PARTY_NOTICES.zh.md)。
